...
public class ShapePlot3D{
	...
	public void autoSetCamera(){
			...
			for ( Shape grid : grids.keySet()){
				...
				double length = grid.getDimension(DimName.R).getLength();
				double start = grid instanceof PolarShape ? 
						graph.loc[0] - length : graph.loc[0];
						length = grid instanceof PolarShape ? 
								grid.getDimension(DimName.R).getLength() : grid.getDimension(DimName.R).getLength();
	
								min_start = Math.min(start, min_start);
								max_length = Math.max(length, max_length);
			}
			System.out.println(((min_start + max_length) / 2) +"  "+ 0 +"  "+(2 * (max_length - min_start)));
	...
	public void plotCurrentConcentrations(SpatialGrid grid){
		...
		int [] current;
		GridGraph graph = getGraph(shape, ArrayType.CONCN);
	...
	/**
	 * @param branch The branch graph (enum ShapePlot3D.Branch).
	 * @param targets The domain to be operated on (enum VoxelTarget).
	 * @param grid
	 * @param type
	 * @param color 
	...
	private double[] getCartLoc(Shape grid, int[] coords, double[] inside)
	{
		...
		if (grid instanceof SphericalShape)
			loc = Vector.unspherify(loc);
		if (grid instanceof CylindricalShape)
			loc = Vector.uncylindrify(loc);
		return loc;
	}
...
//		System.out.println(Arrays.toString(p1));
	...
	/**
	 * Stores all information necessary to visualize a Shape.
	 *
	 */
