...
public abstract class SphericalShape extends PolarShape
{
	...
	{
		Dimension dim = this.getDimension(Dim.R);
	...
	public int[] nbhIteratorNext()
	{
		...
		if ( this._currentNeighbor[0] == this._currentCoord[0] - 1 )
		{
			...
			if ( ! this.increaseNbhByOnePolar(Dim.THETA) )
			{
				...
				if ( ! this.increaseNbhByOnePolar(Dim.PHI) ||
	...
	protected boolean setNbhFirstInNewRing(int ringIndex)
	{
		...
		Dim nbhDimName = this._currentCoord[1] == this._currentNeighbor[1] ?
				Dim.THETA : Dim.PHI;
